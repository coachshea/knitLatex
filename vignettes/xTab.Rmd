...
Simply pass a string into the 'label' option and then use \\ref{'label'} in you
...
bottomrule arguments to \\toprule, \\midrule, and \\bottomrule respectively. When
using booktabs rules, regardless of whether you set booktabs = TRUE or set them
individually, make sure to include \\usepackage{booktabs} in your latex
document.
...
When booktabs is not set, xTab looks for the value of kLat.xTab.booktabs, then
kLat.booktabs, then defaults to FALSE.

### Table Position

If not set, will look for the value of kLat.xTab.position, then defaults to 'ht'
...
column name for the 'rownames' column. When not set will look for value of
kLat.xTab.rows, then klat.rows, then defaults to false.
...
Any arbitrary latex command can be inserted between each row, but the most common
...
### Customize column alginment

Explicilty set the column definitions. If this is set colsep will have no effect
and you must handle column separation within this declaration.

```{r}
xTab(cars, coldef ='rlc|lr')
```
